...
function downloadTemplate() {
  const headers =
    "Name;Artikelnummer;Einheit;Meldebestand;Hersteller;Herstellernummer;Notizen"
  const example = "Kabelrohr 20mm grau;M-001;m;50;Hersteller AG;H-123;Lager A"
    ...
    reader.onload = (e) => {
      ...
      h.forEach((col) => {
        ...
        if (lower.includes("name") || lower.includes("bezeichnung"))
          ...
          lower.includes("nummer") ||
          lower.includes("number") ||
          lower.includes("artikel")
        )
          autoMap[col] = "number"
        else if (lower.includes("einheit") || lower.includes("unit"))
          autoMap[col] = "unit"
        else if (
          lower.includes("melde") ||
          lower.includes("reorder") ||
          lower.includes("minbestand")
        )
          autoMap[col] = "reorderLevel"
        else if (lower.includes("hersteller") || lower.includes("manufacturer"))
          autoMap[col] = "manufacturer"
        else if (lower.includes("herstellernummer") || lower.includes("manufacturernumber"))
          autoMap[col] = "manufacturerNumber"
        else if (
    ...
    const rows: ParsedRow[] = rawRows.map((raw) => {
      ...
      Object.entries(mapping).forEach(([csvHeader, fieldKey]) => {
        ...
        if (fieldKey === "reorderLevel") {
          mapped.reorderLevel = Number(val)
        } else {
      ...
      try {
        const res = await fetch("/api/materials", {
          ...
          body: JSON.stringify({
            ...
            reorderLevel: row.mapped.reorderLevel,
            manufacturer: row.mapped.manufacturer,
            manufacturerNumber: row.mapped.manufacturerNumber,
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Meldebestand</TableHead>
          ...
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <div>
              <p className="text-xl font-bold">
                {results.success} Materialien importiert
              </p>
              {results.failed > 0 && (
