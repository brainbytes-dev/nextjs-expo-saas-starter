...
// ---------------------------------------------------------------------------
// Build batch print HTML (shared logic — same as materials, tool itemType)
// ---------------------------------------------------------------------------
async function buildBatchPrintHtml(
  ...
  const qrDataUrls = await Promise.all(
    items.map((item) => {
      const url = `${window.location.origin}/dashboard/tools/${item.itemId}`
...
<head>
<meta charset="utf-8"/>
<title>Etiketten — Werkzeuge</title>
<style>
...
export default function ToolLabelsPage() {
  ...
  const selectedRows = useMemo(
    () => tools.filter((t) => selected.has(t.id)),
    [tools, selected]
  )

  const allPageSelected = tools.length > 0 && tools.every((t) => selected.has(t.id))
  const somePageSelected = tools.some((t) => selected.has(t.id)) && !allPageSelected
  ...
  const toBarcodeLabelData = useCallback(
    (t: ToolRow): BarcodeLabelData => ({
      name: t.name,
      number: t.number,
      barcode: t.barcode,
      location: t.homeLocationName,
      itemId: t.id,
      itemType: "tool",
    }),
    []
  ...
  async function handlePrintAll() {
    const toPrint = selected.size === 0 ? tools : selectedRows
  ...
  const previewItems = useMemo(() => {
    const rows = selected.size > 0 ? selectedRows : tools.slice(0, 6)
    return rows.slice(0, 12).map(toBarcodeLabelData)
  }, [selected, selectedRows, tools, toBarcodeLabelData])
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={printing || (loading && tools.length === 0)}
            className="gap-1.5"
          >
