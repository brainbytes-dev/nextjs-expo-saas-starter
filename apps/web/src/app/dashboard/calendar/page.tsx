...
// ── Mock calendar data (unchanged from original) ───────────────────────
...
export default function CalendarPage() {
  ...
  const eventsForDate = useMemo(() => {
    ...
    // Overlay maintenance events on calendar
    maintenance.forEach(item => {
      ...
      map.get(key)!.push({
        ...
        date: key,
        color: item.status === "overdue" ? "bg-destructive" : item.status === "this-week" ? "bg-amber-500" : "bg-emerald-500",
      })
    })
    return map
